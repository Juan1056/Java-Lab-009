...
Yes, the blank object is mutable. You can tell this because the object is adjusted inside the riddle method, which means 
it can be changed. 
```
...
The return from #3 returns a value, while the return from #4 does not return any value
```
...
No, they're not aliased. P1 and P2 both refer to different objects. 
```
...
3. Generalize the code so that it works on any string.
    ...
    * You could add square brackets "[]" or curly brackets "{}" to generalize it more.
